...
class Stats():
    ...
    @staticmethod
    def removeOutliersQuantile(df: pd.DataFrame, covColName: str, responseColName: str, cutoff: float = 0.995):
    ...
    def collapseFactorData(data: pd.DataFrame, assetIdColName: str, faultCountsColName: str, covColName: str) -> pd.DataFrame:
        ...
        uncollapsedData = uncollapsedData.sort_values(by=['covFactor', 'assetID'])
        uncollapsedData = uncollapsedData.groupby(['covFactor', assetIdColName])
        uncollapsedData = uncollapsedData.mean().reset_index()
        uncollapsedData.reset_index(drop=True, inplace=True)
        # breakpoint()
        uncollapsedData = uncollapsedData[['covFactor', assetIdColName, faultCountsColName]]
        uncollapsedData.columns = [covColName, assetIdColName, faultCountsColName]
        return uncollapsedData
